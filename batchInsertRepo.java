...
/**
 * Factory responsible for creating step-scoped ItemWriter beans (Bulk Insert component)
...
public class ItemWriterFactory {
     ...
     * This writer handles the bulk insertion logic.
     *
    ...
    public ItemWriter<Map<String, Object>> createWriter(
        ...
        log.info("Creating step-scoped ItemWriter (Bulk Insert handler) for configKey: {}", configKey);
     ...
     * This writer performs efficient bulk inserts using JDBC batch updates.
     *
    ...
    private ItemWriter<Map<String, Object>> buildJdbcWriter(DataSource dataSource, Map<String, String> details, List<FieldMetadata> mappings, String writerName) {
        ...
        log.info("Building JDBC Batch Writer [{}] with SQL: {}", writerName, sql);
        ...
        return new JdbcBatchItemWriterBuilder<Map<String, Object>>()
                ...
                .itemPreparedStatementSetter((itemMap, ps) -> {
                    ...
                    // as defined in the FieldMetadata (set correctly by the ItemReader).
                    if (itemMap == null) {
                         ...
                         return; // Skip setting parameters for this null item
                    }

                    for (int i = 0; i < mappings.size(); i++) {
                        FieldMetadata meta = mappings.get(i);
                        String destFieldName = meta.getDestFieldName();
                        // Get value from map using the destination field name key
                ...
                // .assertUpdates(true) // Optional: Verify that each statement in the batch affected rows
                .build();
    }
